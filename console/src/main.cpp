...
int main()
{
    ...
    main_log.LogInfo() << "Demo starting";
    ...
    while (counter < 30U)
    {
        main_log.LogInfo() << "Loop iteration" << counter
                           << Point{static_cast<int>(counter), static_cast<int>(counter * 2U)};
        ...
        if ((counter % 5U) == 0U)
        {
            sens_log.LogWarn() << "Temperature threshold crossed:" << temperature;
        }
        ...
        if ((counter % 10U) == 0U)
        {
            netw_log.LogError() << "Simulated network error at iteration" << counter;
        }
